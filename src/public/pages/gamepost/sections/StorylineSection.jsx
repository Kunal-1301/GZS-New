function Meta({ label, value }) {
    return (
        <div className="flex justify-between items-center gap-4 group border-b border-[var(--gp-border)] pb-2 last:border-0 last:pb-0">
            <span className="text-[11px] font-bold text-[var(--gp-text-muted)] tracking-widest uppercase">{label}</span>
            <span className="text-[13px] font-black text-[var(--theme-text)] text-right uppercase group-hover:text-[var(--gp-primaryDark)] transition-colors">{value}</span>
        </div>
    );
}
...
export default function StorylineSection({ storyline, info }) {
    return (
        <section className="container-global section-padding grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-20">
            ...
            {/* Metadata card */}
            <div className="bg-white/80 backdrop-blur-xl border border-[var(--gp-border)] rounded-[2rem] p-8 shadow-[0_15px_40px_rgba(0,0,0,0.06)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.12)] gp-card-hover gp-animate-right relative overflow-hidden transition-all duration-300">
                ...
                <div className="pb-4 mb-6 border-b-2 border-[var(--gp-border)] relative z-10">
                    <p className="text-[14px] uppercase tracking-[0.2em] font-black text-[var(--gp-primary)]">Game Details</p>
                </div>
