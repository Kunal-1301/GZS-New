        ...
        <section className="container-global section-padding">
            ...
            <div className="grid lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-2 bg-white/90 backdrop-blur-xl border border-[var(--gp-border)] rounded-3xl p-10 flex flex-col justify-between min-h-[420px] gp-card-hover gp-animate-in shadow-[0_15px_40px_rgba(0,0,0,0.04)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] hover:border-[var(--gp-primary)] transition-all duration-500 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center gap-4 mb-8 border-b border-[var(--gp-border)] pb-6">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-black text-[var(--gp-primary)] uppercase tracking-wide flex items-center gap-2">
                                    <span className="w-2 h-2 rounded-full bg-[var(--gp-primary)] animate-ping"></span>
                                    {community.onlineCount} AGENTS ONLINE
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            {community.messages?.map((message, index) => (
                                <div
                                    ...
                                    className={`p-6 rounded-2xl border-l-4 relative overflow-hidden group ${message.primary ? 'bg-[var(--gp-primary)]/5 border-[var(--gp-primary)]' : 'bg-[var(--gp-bg-page)] border-gray-300'}`}
                                >
                                    <p className={`gzs-body text-base leading-relaxed relative z-10 ${message.primary ? 'text-[var(--theme-text)]' : 'text-[var(--theme-text)] opacity-80'}`}>
                                        <span className={`font-black uppercase tracking-wider text-sm mr-2 ${message.primary ? 'text-[var(--gp-primary)]' : 'text-[var(--theme-text)]'}`}>@{message.author}:</span>{' '}
                                        {message.text}
                                    </p>
                                </div>
                            ))}
                    ...
                    <div className="mt-8 pt-8 border-t border-[var(--gp-border)]">
                        <p className="text-xs font-black uppercase tracking-wide text-[var(--gp-primary)] mb-4 opacity-70">Active Tournament</p>
                        <div className="bg-[var(--gp-primary)]/5 rounded-2xl p-5 border border-[var(--gp-primary)]/20 flex items-center justify-between gap-4">
                            <div>
                                <p className="text-sm font-bold text-[var(--theme-text)]">{community.activeTournament?.name || 'Community Event'}</p>
                                <p className="text-xs text-[var(--theme-text-muted)] mt-1">Apr 20 – Apr 25 · ₹50,000 prize pool</p>
                            </div>
                            <a href={community.activeTournament?.href || '/tournaments/browse'} className="shrink-0 px-4 py-2 bg-[var(--gp-primary)] text-white text-xs font-bold rounded-xl hover:bg-[var(--gp-primaryDark)] transition-all">Register</a>
                        </div>
                    </div>
                    ...
                    <div className="mt-6 flex gap-6 flex-wrap">
                        {(community.stats || []).map(s => (
                            <div key={s.label} className="text-center">
                                <p className="text-lg font-black text-[var(--gp-primary)]">{s.value}</p>
                                <p className="text-xs text-[var(--theme-text-muted)] uppercase tracking-wide">{s.label}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* ================= REVIEW BOX ================= */}
                <div className="bg-[var(--gp-primary)] border border-white/20 rounded-3xl p-10 min-h-[420px] flex flex-col justify-between gp-card-hover gp-animate-in shadow-[0_20px_50px_var(--gp-primary-alpha)] hover:-translate-y-2 transition-transform duration-500 relative overflow-hidden text-white" style={{ animationDelay: '150ms' }}>
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="flex items-center gap-4 mb-8 bg-black/10 p-3 rounded-xl border border-white/20 backdrop-blur-md w-max">
                            ...
                            <p className="text-sm font-black uppercase tracking-widest pr-4 text-white">@GZONEOFFICIAL</p>
                        </div>

                        {/* Role + Rating */}
                        <div className="space-y-8">
                            <div className="flex flex-col gap-3">
                                <span className="text-xs font-black uppercase tracking-tight opacity-80 text-white">SELECT ROLE</span>
                            ...
                            <div className="flex flex-col gap-5">
                                <div className="flex justify-between items-center">
                                    <span className="text-xs font-black uppercase tracking-tight opacity-80 text-white">RATING</span>
