                ...
                {games.map((g, i) => (
                    <Link key={g.slug || i} to={`/games/${g.slug || g.name.toLowerCase().replace(/\s+/g, '-')}`}
                        ...
                        <div className="absolute bottom-0 left-0 w-full p-8 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                            {g.active && (
                                <span className="inline-block px-5 py-2 bg-[var(--gp-primary)] text-white text-xs font-black uppercase tracking-wide rounded-md mb-5 shadow-sm">
                                    TOP PICK
                                </span>
                            )}
                            ...
                            <div className="w-full py-4 bg-white/10 hover:bg-[var(--gp-primary)] text-white text-sm font-black uppercase tracking-wide border border-white/30 hover:border-transparent rounded-xl transition-all duration-300 backdrop-blur-md text-center opacity-0 group-hover:opacity-100 shadow-lg">
                                VIEW PROFILE
                            </div>
                        </div>
                    </Link>
